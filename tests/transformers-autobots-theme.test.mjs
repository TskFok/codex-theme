...
test("Transformers 汽车人主题元数据符合固定契约", () => {
  const theme = JSON.parse(readFileSync(path.join(themeRoot, "theme.json"), "utf8"));
  assert.equal(theme.schemaVersion, 1);
  assert.equal(theme.id, "transformers-autobots-cinematic");
  assert.equal(theme.name, "Transformers Autobots Cinematic");
  assert.equal(theme.brandSubtitle, "AUTOBOT COMMAND NETWORK");
  assert.equal(theme.tagline, "Freedom is built from steel and resolve.");
  assert.equal(theme.statusText, "AUTOBOT SIGNAL ONLINE");
  assert.equal(theme.quote, "ROLL OUT. STAY THE COURSE.");
...
test("Transformers 汽车人主题图片尺寸符合 Codex 背景契约", () => {
...
test("Transformers 汽车人主题 CSS 只包含固定安全规则", () => {
  assert.equal(readFileSync(path.join(themeRoot, "theme.css"), "utf8"), expectedCss);
});

test("Transformers 汽车人主题源目录通过本地简化主题校验", () => {
  const stage = mkdtempSync(path.join(os.tmpdir(), "transformers-autobots-theme-stage-"));
  try {
...
test("Transformers 汽车人主题 ZIP 只有四个根文件且与源目录一致", () => {
