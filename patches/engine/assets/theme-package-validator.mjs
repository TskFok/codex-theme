...
function validateOptionalImageName(theme, names, key) {
  ...
  ) fail(`Local simplified theme ${key} must name an image beside theme.json`);
  return value;
}
...
async function validateSimple(root, names) {
  if (
    (names.length !== 3 && names.length !== 4)
    || !names.includes("theme.json")
    || !names.includes("theme.css")
  ) {
    fail("Local simplified ZIP must contain theme.json, theme.css, its image, and optional sidebar image");
  }
  ...
  const sidebarImage = validateOptionalImageName(theme, names, "sidebarImage");
  if ((sidebarImage && names.length !== 4) || (!sidebarImage && names.length !== 3)) {
    fail("Local simplified ZIP file list does not match sidebarImage");
  }
  if (sidebarImage === theme.image) fail("Local simplified sidebarImage must differ from theme image");
  const [imageBytes, cssBytes, sidebarBytes] = await Promise.all([
    readStableFile(root, theme.image, LIMITS.image),
    readStableFile(root, "theme.css", LIMITS.css),
    sidebarImage ? readStableFile(root, sidebarImage, LIMITS.image) : Promise.resolve(null),
  ]);
  ...
  if (sidebarImage) {
    const sidebarExpectedMedia = /\.png$/i.test(sidebarImage)
      ? "image/png"
      : /\.webp$/i.test(sidebarImage) ? "image/webp" : "image/jpeg";
    if (detectedImageMedia(sidebarBytes) !== sidebarExpectedMedia) {
      fail(`${sidebarImage} content does not match its extension`);
    }
  }
  ...
  if (sidebarImage) payloadBytes.push([sidebarImage, sidebarBytes]);
  return {
    format: "simple",
    image: theme.image,
    safeCssStatus: "validated",
    signatureIgnored: false,
    bytes: new Map(payloadBytes),
  };
}
