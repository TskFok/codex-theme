...
((cssText, artDataUrl, sidebarDataUrl, themeConfig) => {
  ...
  const THEME_VARIABLES = [
    ...
    "--dream-skin-quote", "--dream-skin-art", "--dream-skin-sidebar-art",
  ...
  const sidebarUrl = sidebarDataUrl ? (() => {
    const comma = sidebarDataUrl.indexOf(",");
    const mime = /^data:([^;,]+)/.exec(sidebarDataUrl)?.[1] || "image/png";
    const binary = atob(sidebarDataUrl.slice(comma + 1));
    const bytes = new Uint8Array(binary.length);
    for (let index = 0; index < binary.length; index += 1) bytes[index] = binary.charCodeAt(index);
    return URL.createObjectURL(new Blob([bytes], { type: mime }));
  })() : null;
  ...
  const applyRootState = (root) => {
    ...
    setStyleProperty(root, "--dream-skin-sidebar-art", sidebarUrl ? `url("${sidebarUrl}")` : "none");
  ...
  const cleanup = () => {
    ...
    if (state?.sidebarUrl) URL.revokeObjectURL(state.sidebarUrl);
...
})(__DREAM_SKIN_CSS_JSON__, __DREAM_SKIN_ART_JSON__, __DREAM_SKIN_SIDEBAR_ART_JSON__, __DREAM_SKIN_THEME_JSON__)
