...
function decodeTheme(bytes, label) {
  ...
  if (theme.sidebarImage !== undefined && (
    typeof theme.sidebarImage !== "string"
    || path.basename(theme.sidebarImage) !== theme.sidebarImage
    || !/\.(?:png|jpe?g|webp)$/i.test(theme.sidebarImage)
    || theme.sidebarImage === theme.image
  )) {
    throw new Error(`${label} sidebarImage must reference a distinct image beside theme.json`);
  }
  return theme;
}
...
function normalizedFingerprint(
  ...
  if (sidebarBytes) hash.update("\0sidebarImage\0").update(sidebarBytes);
...
function sourceIdFallbackFingerprint(
  theme,
  imageBytes,
  cssBytes = null,
  licenseBytes = null,
  ...
  const identity = [
    ...
    "sidebarImage", sidebarBytes ? hashBytes(sidebarBytes) : "absent",
...
async function readStoredTheme(directory) {
  try {
    ...
    const sidebarBytes = theme.sidebarImage
      ? await readRegular(path.join(directory, theme.sidebarImage), "Saved theme sidebar image", MAX_IMAGE_BYTES)
      : null;
    ...
    return {
      theme,
      fingerprint: normalizedFingerprint(theme, imageBytes, cssBytes, licenseBytes, sidebarBytes),
      contentFingerprint: runtimeThemeContentFingerprint(theme, imageBytes, cssBytes, sidebarBytes),
      hasOnlyRuntimeFiles,
    };
  } catch {
    return null;
...
async function main() {
  ...
  const sidebarPath = sourceTheme.sidebarImage
    ? path.join(stageRoot, sourceTheme.sidebarImage)
    : null;
  if (sidebarPath) assertContained(stageRoot, sidebarPath, "Imported theme sidebar image");
  const sidebarBytes = sidebarPath
    ? await readRegular(sidebarPath, "Imported theme sidebar image", MAX_IMAGE_BYTES)
    : null;
  ...
  try {
    ...
    const contentFingerprint = runtimeThemeContentFingerprint(
      theme,
      imageBytes,
      cssBytes,
      sidebarBytes,
    ...
    if (theme.sidebarImage) {
      await writeDurableExclusive(path.join(temporary, theme.sidebarImage), sidebarBytes);
    }
    await writeDurableExclusive(
