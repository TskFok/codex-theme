...
THEME_SIDEBAR_IMAGE="$("$NODE" -e '
const value = JSON.parse(process.argv[1]);
const image = value.sidebarImage;
if (image === null || image === undefined) process.exit(0);
if (typeof image !== "string" || !/^[^/\\]+\.(?:png|jpe?g|webp)$/i.test(image)) process.exit(1);
process.stdout.write(image);
' "$STAGE_RESULT")" || fail "Theme staging returned an invalid sidebar image identity."
...
if [ -n "$SAFE_CSS_NAME" ]; then
  if [ -n "$THEME_SIDEBAR_IMAGE" ]; then
    /usr/bin/find "$THEME_DIR" -maxdepth 1 -type f \
      ! -name 'theme.json' ! -name "$THEME_IMAGE" ! -name "$THEME_SIDEBAR_IMAGE" ! -name 'theme.css' -delete
  else
    /usr/bin/find "$THEME_DIR" -maxdepth 1 -type f \
  ...
  if [ -n "$THEME_SIDEBAR_IMAGE" ]; then
    /usr/bin/find "$THEME_DIR" -maxdepth 1 -type f \
      ! -name 'theme.json' ! -name "$THEME_IMAGE" ! -name "$THEME_SIDEBAR_IMAGE" -delete
  else
    /usr/bin/find "$THEME_DIR" -maxdepth 1 -type f \
