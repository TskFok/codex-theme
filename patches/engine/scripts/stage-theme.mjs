...
async function main() {
  ...
  if (sidebarImage !== undefined) {
    if (
      ...
      || path.basename(sidebarImage) !== sidebarImage
      || /[\u0000-\u001f\u007f-\u009f]/u.test(sidebarImage)
      || !/\.(?:png|jpe?g|webp)$/i.test(sidebarImage)
      || sidebarImage === theme.image
    ) {
      throw new Error("Theme sidebarImage must be a distinct image beside theme.json");
    }
  }

  const imagePath = path.resolve(sourceRoot, theme.image);
  const sidebarImagePath = sidebarImage ? path.resolve(sourceRoot, sidebarImage) : null;
  assertContained(sourceRoot, imagePath, "Theme image");
  if (sidebarImagePath) assertContained(sourceRoot, sidebarImagePath, "Theme sidebar image");
  const [image, sidebar, safeCss] = await Promise.all([
    readStableFile(imagePath, "Theme image", MAX_IMAGE_BYTES),
    sidebarImagePath
      ? readStableFile(sidebarImagePath, "Theme sidebar image", MAX_IMAGE_BYTES)
  ...
  if (sidebar?.bytes.length < 1) throw new Error("Theme sidebar image is empty");
  ...
  if (sidebarImage) assertContained(stageRoot, path.join(stageRoot, sidebarImage), "Staged theme sidebar image");
  ...
  if (sidebarImage) await writeExclusive(path.join(stageRoot, sidebarImage), sidebar.bytes);
